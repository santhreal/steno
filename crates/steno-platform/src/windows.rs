...
//! honors `pulse_ms`. Colors/labels come from [`steno_core::resolve_ui`].
...
impl Hotkey {
    ...
    /// Same as `next_event` but checks `shutdown` between poll timeouts
    /// so the event loop can exit promptly on SIGTERM/Ctrl+C.
...
unsafe fn create_chip_window() -> Result<HWND> {
    ...
    if atom == 0 {
        ...
        if err != 1410 {
            bail!("RegisterClassW(StenoStatusChip) failed (Win32 error {err})");
        }
    }
    ...
    if hwnd.is_null() {
        let err = unsafe { GetLastError() };
        bail!("CreateWindowExW(StenoStatusChip) failed (Win32 error {err})");
    }
    Ok(hwnd)
}
