...
use steno_core::{ResolvedUi, resolve_ui};
...
mod backend_tests {
    ...
    fn custom_stage_label_and_timer_flags_resolve() {
        let cfg = UiConfig {
            stages: steno_core::config::UiStages {
...
mod tests {
    ...
    /// Writes only when STENO_DUMP_DIR is set: pure CPU, no X, no mic.
    #[test]
    fn dump_mock_frames() {
        let Ok(dir) = std::env::var("STENO_DUMP_DIR") else {
            return;
        };
