//! macOS OS backends (compile stubs).
//!
//! Real backends will use CGEventTap / CGEvent / NSPanel.
...
/// Caps Lock grab — not implemented on macOS yet.
pub struct Hotkey;

impl Hotkey {
    pub fn grab_caps_lock() -> Result<Self> {
        bail!(
            "macOS backend not implemented yet — use Linux X11 or NullHotkey \
             (CGEventTap path is Phase 4)"
        )
    }
...
/// Progressive emitter — typing not implemented on macOS yet.
pub struct Emitter {
    _mode: OutputMode,
}
...
impl Emitter {
    ...
    pub fn push(&mut self, _chunk: &str) -> Result<()> {
        bail!(
            "macOS backend not implemented yet — use Linux X11 or NullTyper \
             (CGEvent path is Phase 4)"
        )
    }
...
/// Always returns [`NullOverlay`] so headless / embed builds work on macOS.
