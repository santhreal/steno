...
impl ApiClient {
    ...
    pub fn connect(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let stream = UnixStream::connect(path).with_context(|| {
            format!(
                "failed to connect to steno API socket at {} — is the daemon running (`steno start`), and is [api] enabled?",
                path.display()
            )
        })?;
    ...
    pub fn call(&mut self, request: &Request) -> Result<Response> {
        ...
        loop {
            ...
            if n == 0 {
                bail!(
                    "API socket closed before a response arrived — the daemon may have exited; check logs and restart with `steno start`"
                );
            }
