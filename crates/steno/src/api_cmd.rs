//! CLI for the daemon NDJSON API: `steno ping` and `steno api status`.
//!
//! Uses [`steno_core::api::ApiClient`]. Does not start or stop the daemon.
...
fn ensure_api_enabled(config_path: Option<&Path>, socket_override: &Option<PathBuf>) -> Result<()> {
    ...
    if !cfg.api.enabled {
        bail!(
            "[api].enabled is false in config — run `steno config set api.enabled true` and `steno start`"
        );
    }
    Ok(())
}
...
fn connect(
    ...
    let client = ApiClient::connect(&path).with_context(|| {
        format!(
            "daemon API unreachable at {} — start it with `steno start` (and run `steno config set api.enabled true` if disabled)",
            path.display()
        )
    })?;
...
/// `steno ping`: RTT to the daemon API; exit nonzero if down.
pub fn ping(config_path: Option<&Path>, socket: Option<PathBuf>) -> Result<()> {
    ...
    let resp = client
        ...
        .with_context(|| {
            format!(
                "ping failed talking to {} — restart with `steno start`",
                path.display()
            )
        })?;
...
/// `steno api status`: print daemon status JSON from the API socket.
pub fn api_status(config_path: Option<&Path>, socket: Option<PathBuf>) -> Result<()> {
    ...
    let resp = client
        ...
        .with_context(|| {
            format!(
                "status failed talking to {} — restart with `steno start`",
                path.display()
            )
        })?;
...
mod tests {
    ...
    //! live daemon, and must fail closed with a `steno start` hint when the
    //! socket is absent, so `cargo test` never depends on the host daemon.

    use super::*;
    use steno_core::api::{StubHandler, serve_unix_until};
    use std::fs;
    ...
    impl steno_core::api::ApiHandler for NoPongHandler {
        fn ping(&self) -> steno_core::api::ApiResult {
    ...
    fn ping_missing_pong_in_response_bails() {
        ...
        let serve_path = path.clone();
        let handle = thread::spawn(move || {
            let _ = steno_core::api::serve_unix_until(&serve_path, NoPongHandler, Some(stop2));
        });
        for _ in 0..100 {
            if path.exists() && ApiClient::connect(&path).is_ok() {
                break;
    ...
    fn ensure_api_enabled_rejection_carries_exact_cli_command() {
        ...
        assert!(
            err.contains("steno config set api.enabled true"),
