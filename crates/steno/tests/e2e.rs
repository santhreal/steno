//! End-to-end tests: espeak-ng speech → WAV → steno binary → formatted text.
//!
//! Requirements (tests skip with a message when missing):
//! - espeak-ng in PATH (speech synthesis for fixtures)
//! - a sherpa-onnx Parakeet model dir under ~/.local/share/steno/models/ (or STENO_TEST_MODEL)
//!
...
fn test_model() -> Option<PathBuf> {
    if let Ok(m) = std::env::var("STENO_TEST_MODEL").or_else(|_| std::env::var("DICTATE_TEST_MODEL")) {
        return Some(PathBuf::from(m));
    }
    let dir = PathBuf::from(std::env::var_os("HOME")?).join(".local/share/steno/models");
...
/// real ~/.config/steno/config.toml (type_output = true) would make
...
fn steno_bin(args: &[&str], wav: &Path, extra: &[String]) -> Output {
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_steno"));
    cmd.arg(wav);
    // Prefer an explicit --config from args/extra (dictionary fixtures).
    // Otherwise force a hermetic empty config so a user-armed
    // ~/.config/steno/config.toml can never enable typing in CI.
...
fn raw_and_processed(model: &Path, wav: &Path, extra: &[String]) -> (String, String) {
    ...
    let raw_out = steno_bin(&[m[0], m[1], "--raw"], wav, extra);
    assert!(
        raw_out.status.success(),
        "raw run: {}",
        String::from_utf8_lossy(&raw_out.stderr)
    );
    let out = steno_bin(m, wav, extra);
    assert!(
        out.status.success(),
        "stderr: {}",
...
fn e2e_commands_and_formatting() {
    ...
    let Some(model) = test_model() else {
        eprintln!("SKIP: no model in ~/.local/share/steno/models");
        return;
    };
    let tmp = std::env::temp_dir().join("steno-e2e-1");
...
fn e2e_scratch_and_dictionary() {
    ...
    let tmp = std::env::temp_dir().join("steno-e2e-2");
    std::fs::create_dir_all(&tmp).unwrap();
    ...
    // Last --config wins over hermetic empty config inside steno_bin().
...
fn e2e_quotes_paragraph_and_names() {
    ...
    let tmp = std::env::temp_dir().join("steno-e2e-3");
    std::fs::create_dir_all(&tmp).unwrap();
    let wav = speak(
        &tmp,
        "t3.wav",
...
fn typing_is_fail_closed_without_config_arming() {
    ...
    let tmp = std::env::temp_dir().join("steno-e2e-blocker");
    std::fs::create_dir_all(&tmp).unwrap();
    let cfg = hermetic_config(&tmp);
    let out = Command::new(env!("CARGO_BIN_EXE_steno"))
        .arg("--type")
        .arg("--config")
        .arg(&cfg)
        .output()
